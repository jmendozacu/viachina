...
// <<< fix for PSystem_CatalogAjax extension (not updated layered navigation data when pager was used and then layered navigation filter applied)
if(strpos($html, 'adj-nav-container') === false)
{
...
// >>> fix for PSystem_CatalogAjax extension
        }
        else {
            $html = '<div id="adj-nav-container">'
                  . $html
	...
	public function getCategories($categories)
    {
        ...
        foreach($categories as $category) {
            ...
            if($category->hasChildren()) {
                $children = Mage::getModel('catalog/category')->getCategories($category->getId(), 1, false, true);
                $allCategories = array_merge($allCategories, $this->getCategories($children));
            }
        }
