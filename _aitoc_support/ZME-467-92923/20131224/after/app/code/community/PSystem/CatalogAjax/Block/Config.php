...
// <<< fix for AdjustWare_Nav extension
if(Mage::getConfig()->getModuleConfig('AdjustWare_Nav')->is('active', 'true'))
{
    if($dataKey == 'navfilter' || $dataKey == 'navclear' || $dataKey == 'navremove')
    {
        if (!isset($newConfig[$confKey]))
        {
		    $newConfig[$confKey] = array();
        }
	    $newConfig[$confKey][$dataKey] = '';
    }
}
// >>> fix for AdjustWare_Nav extension
			}
