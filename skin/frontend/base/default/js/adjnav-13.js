...
function adj_nav_show_products(transport)
{
    ...
    if (transport && transport.responseText){
     ...
     $$('div.category-head').each(function(el) {
            el.select('h2').each(function(e) {
...
function adj_nav_get_aitanswer()
{
	var aitanswer = '';
...
function adj_nav_make_request()
{
    ...
    new Ajax.Request($('adj-nav-ajax').value + '?' + $('adj-nav-params').value + adj_nav_get_aitanswer(), 
...
function adj_nav_prepare_params()
{
    ...
    $('adj-nav-params').value = Object.toQueryString(params);//.gsub('%2B', '+');
    /**
     * @author ksenevich@aitoc.com
     */
...
//    var items = $('adj-nav-container').select('.pages a', '.view-by a');
    var items = $('adj-nav-container').select('.pages a', '.view-mode a', '.view-by a', '.sort-by a');
    var i, n = items.length;
    for (i=0; i<n; ++i){
...
function adj_nav_create_slider(width, from, to, max_price,sKey) 
{
...
function adj_nav_page_autoload_make_request(isChange)
{
    ...
    {   
        ...
        new Ajax.Request($('adj-nav-ajax').value + '?' + $('adj-nav-params').value + '&no_cache=true' + pageParam + adj_nav_get_aitanswer(), 
...
function adj_nav_page_autoload_products_show(transport)
{
    ...
    {        
        ...
        var upd = [];
        var mode = '';
        if ($$('.listing-item').size())
        {
            mode = 'list';
        } else if ($$('.grid-row').size())
        {
            mode = 'grid';
        }
...
{
    var pager = Element.select(container, '.pager');
    if (pager.size())
    {
        $$('.pager').each(function(item){
            Element.update(item, pager.first().innerHTML);
        });
    }
    var toolbar = Element.select(container, '.adjnav-page-autoload-nextpage');
    if (toolbar.size())
    {
        $$('.adjnav-page-autoload-nextpage').each(function(item){
            item.value = toolbar.first().value;
        });
    }
...
function adj_nav_page_autoload_products_insert(el, mode)
{
    if (mode == 'list')
    {
        $$('.listing-item').each(function(item){
            item.removeClassName('last');
        });
        Element.select(el, '.listing-item').each(function(item){
            $$('.listing-item').last().insert({after:item});
        });
    }
    if (mode == 'grid')
    {
        adj_nav_page_autoload_grid_update(el);
        $$('.grid-row').each(function(item){
...
{    
    ...
    function _insert()
    {
        $R(0, count, true).each(function(index){
            ...
            $$('.grid-row').last().insert({bottom:item});
        });
        var row = Element.select(el, '.grid-row').first();
        if (Element.select(row, '.item').size() > 0)
        {
            $$('.grid-row').last().insert({after:row});
        } else {
            Element.remove(row);
        }
        if (Element.select(el, '.grid-row').first())
        {
            _insert();
        } else {
            return;
    ...
    {
        Element.select(el, '.grid-row').each(function(item){
            $$('.grid-row').last().insert({after:item});
        });
    } else {
        _insert();
    }
