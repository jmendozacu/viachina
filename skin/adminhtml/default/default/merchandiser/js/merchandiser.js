...
jQuery(document).ready(function() {
	initFunction();
	//jQuery('#massproduct_skus').width(jQuery('#infinite_scroll').width() - 10);
});
...
/*	----------------- functions ---------------		*/
	function initFunction(){
		observerMultiSelect();
		setSortable();
		observeCategoryItemsButtons();
		observeCategoryMovetopButtons();
		observeMoreImageThumbnails();
		preventMoreImageLink();
		updateInputPositions();
	}

	function observeCategoryItemsButtons() {
	...
	function observeCategoryMovetopButtons() {
	...
	function hideDuplicates() {
		...
		jQuery('.search-results .dragbox').each( function(index) {
			item = jQuery(this);
			prodId = getProdIdFromClass(item.attr('class'), mandatory)
			if (prodId) {
				...
				if (0 < duplItem.length) {
					...
					jQuery('.move-top').unbind('click').click( function() {
						...
						//jQuery('#infinite_scroll').find('#sk' + this.id).remove();
						...
						liHTML += '</li>';
						jQuery(liHTML).prependTo('#infinite_scroll');
						//$$(".input-product-position")[0].value = 1;
	...
	function toggleDragboxEmpty(enable) {
	...
	function removeDuplicatesAfterAjax() {
		removeDups();
	    /*jQuery('.productid').each(function(index){
	        var item = jQuery(this);
	        var proId = getProdIdFromClass(item.attr('class'), 'productid');
	        if (proId) {
	            if(jQuery('.productid'+proId).size() > 1){
	            		jQuery('.productid'+proId).eq(1).remove();
	            }
	        }
	    });*/
	}

    function getProdIdFromClass(classAttr, mandatory) {
        var ret = false;
    ...
    function preventMoreImageLink(){
    	jQuery('.product-image-more-item a').unbind('click').click(function (e) {
		    e.preventDefault();
		});
    }

    function observeMoreImageThumbnails(){
	...
	function removeMassProducts(){
		...
		if( countElements(sku_error) ) {
    		jQuery('#massproductresult').append('The following SKUs were not found: ');
    		jQuery('#massproductresult').append(sku_error.join());
			jQuery('#massproductresult').append('<br/>');
    	}
    	if( countElements(sku_removed) ) {
    		jQuery('#massproductresult').append('The following SKUs were removed: ');
	...
	function openMassProductAssignment(){
	...
	function countElements(sku_array){
		var maxindex = 0;
	...
	function addMassProducts(asid,productinfourl){
	    ...
	    var sku_array = jQuery.trim(jQuery('#massproduct_skus').val()).split("\n");
	    var maxindex = countElements(sku_array);
	    ...
	    var addmassfunction = function(sku_array,index){
	        if( jQuery('#merchandiser-categories #sku-'+sku_array[index]).length ) {
	            ...
	            request.done(function(msg) {
	                ...
	                if (index < maxindex) {
	                	...
	                	if( countElements(sku_added) ) {
	                		jQuery('#massproductresult').append('The following SKUs were added: ');
	                		jQuery('#massproductresult').append(sku_added.join());
                			jQuery('#massproductresult').append('<br/>');
                			hideMassProducts();
	                	}
	                	if( countElements(sku_existed) ) {
	                		jQuery('#massproductresult').append('The following SKUs were existed is this category: ');
	    ...
	    addmassfunction(sku_array,0);
	}

	function hideMassProducts(){
		isRunning = false;
