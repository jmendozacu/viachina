...
abstract class Madia_Adyen_Model_Adyen_Abstract extends Mage_Payment_Model_Method_Abstract {
    ...
    protected function _processResponse(Varien_Object $payment, $response, $request = null) {
        ...
        switch ($responseCode) {
            ...
            case "RedirectShopper":
				$enable_3dsecure = $this->_getConfigData('enable_3dsecure', 'adyen_cc');
				if ($enable_3dsecure) {
					...
					$errorMsg = Mage::helper('adyen')->__('The payment cannot be processed, because 3D-secure card validation is disabled for this webshop.');
    ...
    protected function _getAdyenUrls() {
        $test = array(
            ...
            'wsdl' => Mage::getModuleDir('etc', 'Madia_Adyen') . DS . 'Payment.wsdl'
        );
        $live = array(
