...
class Madia_Adyen_Model_Process extends Mage_Core_Model_Abstract {
	    	...
	    	if($total_result_checksum == $checksum) {
		    	...
		    	if (empty($orderExist)) {
			    	...
			    	if($result == 'APPROVED') {
			    		...
			    		$order->setAdyenEventCode(Madia_Adyen_Model_Event::ADYEN_EVENT_POSAPPROVED);
    ...
    protected function _addAdyenAttributes(Varien_Object $order, $response, $updateAdyenStatus = true) {
        ...
        if(!(substr($order->getAdyenEventCode(), 0, 13) == Madia_Adyen_Model_Event::ADYEN_EVENT_AUTHORISATION && $authResult == Madia_Adyen_Model_Event::ADYEN_EVENT_AUTHORISED)){
