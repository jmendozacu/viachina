...
class Madia_Adyen_Block_Redirect extends Mage_Core_Block_Abstract {
    ...
    protected function _toHtml() {
    	...
    	if($payment->getCode() == "adyen_pos") {
    		...
    		$url = urlencode(Mage::getBaseUrl()."adyen/process/successPos");
    		
    		// detect ios or andoid
    		$ua = strtolower($_SERVER['HTTP_USER_AGENT']);
    		$android = stripos($ua,'android');
    		//$android = true; // FOR TESTING!
    		...
    		// add recurring
    		$extra_paramaters .= "&recurringContract=".$adyFields['recurringContract']."&shopperReference=".$adyFields['shopperReference']. "&shopperEmail=".$adyFields['shopperEmail'];
