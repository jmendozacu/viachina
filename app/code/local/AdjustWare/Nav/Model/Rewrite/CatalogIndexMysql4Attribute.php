...
class AdjustWare_Nav_Model_Rewrite_CatalogIndexMysql4Attribute extends Mage_CatalogIndex_Model_Mysql4_Attribute
{
    ...
    protected function _createTemporary($tableName)
    {
        $query = 'CREATE TEMPORARY TABLE IF NOT EXISTS tmp_table_2 LIKE ' . $tableName . ';';
        $query .= 'INSERT INTO tmp_table_2 SELECT * FROM ' . $tableName . ';';
	...
	$configurableSelect->reset(Zend_Db_Select::GROUP);
        ...
        foreach ($configurableFrom as $alias => $tableInfo)
        {
            ...
            if (0 === strpos($alias, 'tmp_table'))
            {
                $this->_createTemporary($tableInfo['tableName']);
                $tableInfo['tableName'] = 'tmp_table_2';
                $tableInfo['joinCondition'] = str_replace('tmp_table.entity_id', 'tmp_table_2.entity_id', $tableInfo['joinCondition']);
                unset($configurableFrom[$alias]);
                $attributeJoins['tmp_table_2'] = $tableInfo;
            }
        }

        $configurableWhere = $configurableSelect->getPart(Zend_Db_Select::WHERE);
        $newWhere = array();
        foreach ($configurableWhere as $where)
        {
            if (strpos($where, 'tmp_table'))
            {
                $newWhere[] = str_replace('tmp_table', 'tmp_table_2', $where);
            }
        }

        if(count($newWhere))
        {
            $configurableSelect->setPart(Zend_Db_Select::WHERE, $newWhere);
        }
