...
class Custom_SearchByName_Model_Layer extends Mage_Catalog_Model_Layer
{
    ...
    public function getProductCollection()
    {
        if (isset($this->_productCollections[$this->getCurrentCategory()->getId()])) {
            ...
            $collection = Mage::getResourceModel('searchbyname/collection');
    ...
    {
        $attributes = Mage::helper('searchbyname')->getQuery()->getAttributes();
        $productCollection = array();
        foreach($attributes as $code => $attr){
            $proCollection = Mage::getModel('catalog/product')->getCollection()->addFieldToFilter(array(array('attribute'=>$code, 'eq'=>$attr['value'])))->addStoreFilter();
            if($proCollection){
                foreach($proCollection as $product){
                    array_push($productCollection, $product->getId() );
                }
            }
        }

        $preparedList = array_count_values($productCollection) ;
        arsort($preparedList);

        foreach($preparedList as $id => $count){
            $productIds[] = $id;
            $matchOptions[$id] = (int)($count/count($attributes)*100);
        }

        Mage::helper('searchbyname')->getQuery()->setData('match_list', $matchOptions);

        $collection
            ->addAttributeToSelect(Mage::getSingleton('catalog/config')->getProductAttributes())
            ->addSearchFilter($productIds)
            ...
            ->addStoreFilter()
            ->addUrlRewrite();
        $productIds_sql = implode(",", $productIds);
        $collection->getSelect()->order(new Zend_Db_Expr("FIELD (e.entity_id, {$productIds_sql})"));
