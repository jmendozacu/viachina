...
class MW_Dailydeal_Adminhtml_DealschedulerController extends Mage_Adminhtml_Controller_Action
{
    ...
    public function editAction()
    {
        ...
        if ($model->getId() || $id == 0) {
            ...
            $products = MW_Dailydeal_Model_Dealschedulerproduct::getInstance()->getProductOptionArray($id);
            ...
            $this->_addContent($this->getLayout()->createBlock(new MW_Dailydeal_Block_adminhtml_Dealscheduler_Edit()))
                    ->_addLeft($this->getLayout()->createBlock(new MW_Dailydeal_Block_adminhtml_Dealscheduler_Edit_Tabs()));
    ...
    public function saveAction()
    {
        ...
        try {
            ...
            $model = MW_Dailydeal_Model_Dealscheduler::getModel();
            ...
            if (!empty($data['links'])) {
                ...
                $product_scheduler = MW_Dailydeal_Model_Dealschedulerproduct::getModel();
            ...
            if (!empty($data['rule_auto_generate_deal'])) {
                $count = MW_Dailydeal_Model_Business::generalDeal($model->getId());
    ...
    public function gridProductAction()
    {
        ...
        $response = $this->getLayout()->createBlock(new MW_Dailydeal_Block_adminhtml_Dealscheduler_Edit_Product_Grid())->getHtml();
    ...
    protected function decodeDeal($str_encode)
    {
        ...
        foreach ($str_products as $value) {
            ...
            foreach ($deal_values as $deal_key => $deal_string) {
                $deal_value = array();
                $deal_value = MW_Dailydeal_Helper_Toolasiaconnect::exploseEqualToArray($deal_string);
