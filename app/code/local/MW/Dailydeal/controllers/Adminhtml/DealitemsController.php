...
class MW_Dailydeal_Adminhtml_DealitemsController extends Mage_Adminhtml_Controller_Action
{
    ...
    public function indexFilterAllAction(){
        Mage::app()->getRequest()->setParam('filter', '');
        $this->_forward('index');
    }

    public function indexFilterRunningAction(){
        $filter = 'active=1';
    ...
    public function saveAction()
    {
        ...
        try {
            ...
            $model = MW_Dailydeal_Model_Dailydeal::getModel()
                    ->setData($data)
                    ->setId($id)
                    ->save();
    ...
    {
        $response = $this->getLayout()->createBlock(new MW_Dailydeal_Block_Adminhtml_Sales_Order_Grid())->getHtml();
        $this->getResponse()->setBody($response);
    }
