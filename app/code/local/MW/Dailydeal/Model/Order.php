...
class MW_Dailydeal_Model_Order extends Mage_Core_Model_Abstract
{

    /**
     * @return MW_Dailydeal_Model_Order
     */
    ...
    public function cancelOrderEnableProduct($observer)
    {
        ...
        foreach ($items as $item) {
            ...
            if ($model_deal->getId() && $model_deal->getData('product_id') == $item->getData('product_id')) {
                MW_Dailydeal_Model_Product::getInstance()->enableProductByDealId(array($model_deal->getId()));
            }
        }
    }
}