...
class MW_Dailydeal_Model_Dealscheduler extends Mage_Core_Model_Abstract
{
    ...
    /**
     * @return MW_Dailydeal_Model_Dealscheduler
     */
    ...
    public function getThreadStartDateTime($index, $thread_start_date_time)
    {
        ...
        if ($now_time >= $data_time) {
            ...
            $data[$index] = MW_Dailydeal_Helper_Toolasiaconnect::increaseTime($data[$index], 3, 1, 'Y-m-d H:i:s');
        }
    ...
    public function generalDeal($product, $index, $thread_start_date_time)
    {
        try {
            ...
            $model_deal = MW_Dailydeal_Model_Dailydeal::getInstance();
            ...
            $data_default['featured'] = MW_Dailydeal_Model_Status::STATUS_FEATURED_DISABLED;
            $data_default['disable_product_after_finish'] = MW_Dailydeal_Model_Status::STATUS_PRODUCT_DISABLED;
            $data_default['status'] = MW_Dailydeal_Model_Status::STATUS_ENABLED;
            ...
            $end_date_time = MW_Dailydeal_Helper_Toolasiaconnect::increaseTime($start_date_time, 1, $data_default['deal_time'], 'Y-m-d H:i:s');
    ...
    public function getStatusTime()
    {
        ...
        // 1 DISABLE
        if ($model_deal->getData('status') == MW_Dailydeal_Model_Status::STATUS_DISABLED) {
            return MW_Dailydeal_Model_Status::STATUS_TIME_DISABLED;
        }

        // 2 ENABLE, RUNNING
        elseif ($start_date_time <= $timestamp AND $timestamp <= $end_date_time) {
            return MW_Dailydeal_Model_Status::STATUS_TIME_RUNNING;
        }

        // 3 ENDED
        elseif ($end_date_time < $timestamp) {
            return MW_Dailydeal_Model_Status::STATUS_TIME_ENDED;
        }

        // 4 QUEUED
        if ($timestamp < $start_date_time) {
            return MW_Dailydeal_Model_Status::STATUS_TIME_QUEUED;
        }
    ...
    public function getDealPrice($model_product)
    {
        ...
        if (MW_Dailydeal_Helper_Toolasiaconnect::isValidPattern(self::PATTERN_DEAL_PRICE_1, $value)) {
            $temp = explode('-', $value);
            $price_down = rand($temp[0], $temp[1]);
            $result = $price - $price_down;
        } elseif (MW_Dailydeal_Helper_Toolasiaconnect::isValidPattern(self::PATTERN_DEAL_PRICE_2, $value)) {
            $value = str_replace('%', '', $value);
            $temp = explode('-', $value);
            $percent = rand($temp[0], $temp[1]);
            $price_down = ($price * $percent) / 100;
            $result = $price - $price_down;
        } elseif (MW_Dailydeal_Helper_Toolasiaconnect::isValidPattern(self::PATTERN_DEAL_PRICE_3, $value)) {
            $temp = explode(',', $value);
    ...
    public function getDealQty($model_product)
    {
        ...
        if (MW_Dailydeal_Helper_Toolasiaconnect::isValidPattern(self::PATTERN_DEAL_QTY_BLANK, $value)) {
            $result = 0;
        }
        if (MW_Dailydeal_Helper_Toolasiaconnect::isValidPattern(self::PATTERN_DEAL_QTY_1, $value)) {
            $temp = explode('-', $value);
            $result = rand($temp[0], $temp[1]);
        }
        if (MW_Dailydeal_Helper_Toolasiaconnect::isValidPattern(self::PATTERN_DEAL_QTY_2, $value)) {
            $temp = explode(',', $value);
            $key_random = array_rand($temp);
