...
class MW_Dailydeal_Model_Dailydeal extends Mage_Core_Model_Abstract
{
    ...
    /**
     * @return MW_Dailydeal_Model_Dailydeal
     */
    ...
    protected function validate()
    {
        ...
        $model_origin = MW_Dailydeal_Model_Dailydeal::getModel()->load($this->getId());
        ...
        if ($this->getData('status') != MW_Dailydeal_Model_Status::STATUS_DISABLED) {
...
//        }
    ...
    {
        $old_deal = MW_Dailydeal_Model_Dailydeal::getModel();
    ...
    protected function convertStoreViewToString()
    {
        if(!Mage::app()->isSingleStoreMode()){
            $data = $this->getData();
            $help_toolac = MW_Dailydeal_Helper_Toolasiaconnect::getInstance();
    ...
    protected function setDisableProduct()
    {
        ...
        if ($model_deal->getData('disable_product_after_finish') == MW_Dailydeal_Model_Status::STATUS_PRODUCT_DONE) {
            $flag_status_time = $this->getStatusTime();
            if ($flag_status_time == MW_Dailydeal_Model_Status::STATUS_TIME_ENDED || $flag_status_time == STATUS_TIME_DISABLED) {
                $data = $this->getData();
    ...
    protected function setExpire()
    {
        ...
        if (isset($id)) {
            ...
            $model_deal_temp = MW_Dailydeal_Model_Dailydeal::getModel()->load($id);
        ...
        $expire = $flag_qty ? MW_Dailydeal_Model_Status::STATUS_EXPIRE_FALSE : MW_Dailydeal_Model_Status::STATUS_EXPIRE_TRUE;
     ...
     * @return MW_Dailydeal_Model_Mysql4_Dailydeal_Collection
     */
    public function loadListDeals($condition = array())
    {
        ...
        if ($condition['featured'] == true) {
            $collection->addFieldToFilter('featured', MW_Dailydeal_Model_Status::STATUS_FEATURED_ENABLED);
        }
    ...
    public function getStatusTime()
    {
        ...
        // 2 ENABLE, RUNNING
        elseif ($start_date_time <= $timestamp AND $timestamp <= $end_date_time AND $expire == MW_Dailydeal_Model_Status::STATUS_EXPIRE_FALSE) {
            return MW_Dailydeal_Model_Status::STATUS_TIME_RUNNING;
        }

        // 3 ENDED
        elseif ($end_date_time < $timestamp || $expire == MW_Dailydeal_Model_Status::STATUS_EXPIRE_TRUE) {
    ...
    {
        if ($model_product->getData('type_id') == Mage_Catalog_Model_Product_Type::TYPE_GROUPED) {
            $price = MW_Dailydeal_Model_Product::getMinPriceProductGrouped($model_product);
            $model_product->setData($column, $price);
        } elseif ($model_product->getData('type_id') == Mage_Catalog_Model_Product_Type::TYPE_BUNDLE) {
            $price = MW_Dailydeal_Model_Product::getMinPriceProductBundle($model_product);
            $model_product->setData($column, $price);
        }
    }
