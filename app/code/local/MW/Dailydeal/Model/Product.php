...
class MW_Dailydeal_Model_Product extends Mage_Core_Model_Abstract
{

    /**
     * @return MW_Dailydeal_Model_Product
     */
    ...
    public function disableProductByDealId($deal_ids = array())
    {
        ...
        foreach ($deal_ids as $id) {
            ...
            if ($model_deal->getId()) {
                ...
                if ($model_deal->getStatusTime() == MW_Dailydeal_Model_Status::STATUS_TIME_ENDED) {

                    if ($model_deal->getData('disable_product_after_finish') == MW_Dailydeal_Model_Status::STATUS_PRODUCT_ENABLED) {
        ...
        foreach ($deal_ids as $id) {
            ...
            if ($model_deal->getId()) {
                if ($model_deal->getData('disable_product_after_finish') == MW_Dailydeal_Model_Status::STATUS_PRODUCT_DONE) {
                    ...
                    $model_deal->setData('disable_product_after_finish', MW_Dailydeal_Model_Status::STATUS_PRODUCT_ENABLED);
                    $model_deal->save();
                }
            }
        }
