 ...
 * - Auto
 */
class MW_Dailydeal_Model_Business extends Mage_Core_Model_Abstract
{
    ...
    {
        $model_deal = MW_Dailydeal_Model_Dailydeal::getModel()->load($deal_id);
    ...
    {
        $model_deal_scheduler = MW_Dailydeal_Model_Dealscheduler::getModel()->load($deal_scheduler_id);
        $model_product_scheduler = MW_Dailydeal_Model_Dealschedulerproduct::getModel();
        $model_deal = MW_Dailydeal_Model_Dailydeal::getModel();
        ...
        $to_time = MW_Dailydeal_Helper_Toolasiaconnect::increaseTime($now_time, 5, $model_deal_scheduler->getData('number_day'), 'Y-m-d H:i:s');

        if ($model_deal_scheduler->getStatusTime() != MW_Dailydeal_Model_Status::STATUS_TIME_RUNNING) {
        ...
        while (in_array(true, $thread_have_time) && $number_product > 0) {   // Have time and product
            for ($i = 0; $i < $theads; $i++) {   // Muity Thread Deal
                ...
                if ($count_generate_deal == MW_Dailydeal_Model_Status :: DEAL_SCHEDULER_GENERATE_LIMIT_AMOUNT) {
                    return $count_generate_deal;
                }
            }
        }
    ...
    {
        $flag_send_mail_on = MW_Dailydeal_Helper_Data::getConfigAllowSendAdminMail();
        ...
        $template_id = MW_Dailydeal_Helper_Data::getConfigTemplateIdNoDeal();

        $sender = 'sales';
        $receive = MW_Dailydeal_Helper_Data::getConfigAdminMail();
...
//            throw new Exception(Mage::helper('dailydeal')->__("Email can not send!"));
    ...
    public function autoDisableProduct()
    {
        ...
        $collection_deal = MW_Dailydeal_Model_Dailydeal::getModel()->getCollection()
                ->addFieldToFilter('status', MW_Dailydeal_Model_Status::STATUS_ENABLED)
                ->addFieldToFilter('disable_product_after_finish', MW_Dailydeal_Model_Status::STATUS_PRODUCT_ENABLED)
        ...
        foreach ($collection_deal as $model_deal) {
            ...
            $model_deal->setData('disable_product_after_finish', MW_Dailydeal_Model_Status::STATUS_PRODUCT_DONE);
            $model_deal->save();
        }
    }
    ...
    public static function autoUpdateDealActive()
    {
        $collection = MW_Dailydeal_Model_Dailydeal::getModel()->getCollection()
                ->addFieldToFilter('active', array(MW_Dailydeal_Model_Status::STATUS_TIME_QUEUED, MW_Dailydeal_Model_Status::STATUS_TIME_RUNNING));
    ...
    public function autoGenerateDeal()
    {
        ...
        $collection = MW_Dailydeal_Model_Dealscheduler::getModel()->getCollection()
                ->addFieldToFilter('status', MW_Dailydeal_Model_Status::STATUS_ENABLED)
                ->addFieldToFilter('start_date_time', array('to' => $now))
                ->addFieldToFilter('end_date_time', array('from' => $now));

        foreach ($collection as $model) {
            MW_Dailydeal_Model_Business::generalDeal($model->getId());
        }
    }
    ...
    public static function autoSendMail()
    {
        ...
        $tomorrow = MW_Dailydeal_Helper_Toolasiaconnect::increaseTime($now, 5, 1, 'Y-m-d H:i:s');
        ...
        $flag_send_mail_on = MW_Dailydeal_Helper_Data::getConfigAllowSendAdminMail();
        $flag_have_deal = MW_Dailydeal_Model_Dailydeal::getModel()->isHaveDealRunning($condition);
        $flag_send_mail = MW_Dailydeal_Helper_Data::getConfigSendMailAdminNotification();

        if ($flag_send_mail_on) {

            if (!$flag_have_deal && $flag_send_mail) {
                // Not deal, send mail
                MW_Dailydeal_Model_Business::sendMailAdminNotification();
                $result = 1;
            }
        }
