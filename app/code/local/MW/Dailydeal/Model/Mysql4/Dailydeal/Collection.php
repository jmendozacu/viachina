...
class MW_Dailydeal_Model_Mysql4_Dailydeal_Collection extends Mage_Core_Model_Mysql4_Collection_Abstract
{
    ...
    public function getActiveDailydeal($product = null)
    {
        ...
        $product->setMW_Dailydeal($active_dailydeal);
        return $active_dailydeal;
    }
    ...
    public function getConfigSortBy(){
        $sort = MW_Dailydeal_Helper_Data::getConfigSortBy();
        if($sort == MW_Dailydeal_Model_Source_Sortby::SORT_BY_FEATURED_ENDDATETIME ){
            $this->addAttributeToSort('featured', 'ASC')
                 ->addAttributeToSort('end_date_time', 'ASC');
        }elseif($sort == MW_Dailydeal_Model_Source_Sortby::SORT_BY_RANDOM){
            $this->getSelect()->order('rand()');
        }elseif($sort == MW_Dailydeal_Model_Source_Sortby::SORT_BY_FEATURED_RANDOM){
            $this->addFieldToFilter('featured', MW_Dailydeal_Model_Status::STATUS_FEATURED_ENABLED);
            $this->getSelect()->order('rand()');
        }
        return $this;
    }
