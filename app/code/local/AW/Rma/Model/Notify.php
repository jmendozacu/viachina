...
class AW_Rma_Model_Notify extends Mage_Core_Model_Abstract
{
    ...
    protected function _notifyByStatus($rmaRequest, $commentText = null, $isFrontend = false)
    {
        if ($this->_isGenerallyEnabled()) {
            ...
            $oldDesign = Mage::getDesign()->setAllGetOld(
                array(
            ...
            if ($_status->getData() == array()) {
                throw new Exception('Can\t load request status');
            }

            /* messages for RMA store */
            $_status->storeTemplateFromRma($rmaRequest->getStoreId());
            ...
            $departmentAddress = Mage::helper('awrma/config')->getDepartmentAddress($rmaRequest->getStoreId());
            ...
            if ($this->_isEnabledForCustomer() && $_status->getToCustomer()) {
                $rmaRequest->setData('notify_rma_address', nl2br($departmentAddress));
                ...
                $emailTemplate = Mage::getModel('awrma/email_template')
                    ...
                    ->sendEmail(
                        $_sender, $rmaRequest->getCustomerEmail(), $rmaRequest->getCustomerName(),
                        array(
                            'request' => $rmaRequest,
                            'store'   => $_store,
                            'subject' => $_subject,
            ...
            if ($this->_isEnabledForAdmin() && $_status->getToAdmin() && $isFrontend) {
                ...
                $emailTemplate = Mage::getModel('awrma/email_template')
                    ...
                    ->sendEmail(
                        $_sender, Mage::helper('awrma/config')->getDepartmentEmail($rmaRequest->getStoreId()),
                        Mage::helper('awrma/config')->getDepartmentDisplayName($rmaRequest->getStoreId()),
                        array(
                            'request' => $rmaRequest,
                            'store'   => $_store,
                            'subject' => $_subject,
            ...
            if ($_status->getToChatbox()) {
                $rmaRequest->setData('notify_rma_address', $departmentAddress);
                $emailTemplate = Mage::getModel('awrma/email_template')
                    ->setDesignConfig(array('area' => 'frontend', 'store' => $rmaRequest->getStoreId()))
                ...
                Mage::helper('awrma/comments')->postComment($rmaRequest->getId(), $processedTemplate, array(), false);
            }
