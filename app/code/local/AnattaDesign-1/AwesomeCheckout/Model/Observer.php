...
class AnattaDesign_AwesomeCheckout_Model_Observer {
	...
	public function layoutUpdate( $observer ) {
		...
		if ( Mage::helper( 'anattadesign_awesomecheckout' )->isVirtualOnly() ) {
			$node = new Mage_Core_Model_Config_Element( '<anattadesign_awesomecheckout_virtual_support module="anattadesign_awesomecheckout"><file>anattadesign_awesomecheckout/virtual_support.xml</file></anattadesign_awesomecheckout_virtual_support>' );
			$observer->getEvent()->getUpdates()->appendChild( $node );
		}
	}
	...
	public function checkAwesomeCheckoutVersion() {
		$contents = file_get_contents( 'http://api.anattadesign.com/awesomecheckout/1alpha/status/latestVersion' );
		$latest = json_decode( $contents );

		if ( $latest->status == "success" ) {

			// Don't do anything if we are on latest, this prevents duplicate notifications
			if ( version_compare( $latest->latestVersion, Mage::getStoreConfig( 'anattadesign_awesomecheckout_latest_checked_version' ), 'eq' ) )
				return;
			...
			if ( version_compare( $latest->latestVersion, $version, '>' ) ) {
				Mage::getModel( 'adminnotification/inbox' )
						->setSeverity( Mage_AdminNotification_Model_Inbox::SEVERITY_NOTICE )
						->setTitle(Mage::helper('anattadesign_awesomecheckout')->__("Awesome Checkout %s is now available", $latest->latestVersion))
						...
						->save();
				Mage::getModel( 'core/config' )->saveConfig( 'anattadesign_awesomecheckout_latest_checked_version', $latest->latestVersion );
			}
		}
	}
